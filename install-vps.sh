...
echo "=== Starting n8n and Portainer Installation ==="
echo "This script will install Docker, Docker Compose, and set up n8n with Caddy and Portainer"
...
# 2. Clone the repository - Commented out as not needed
...
if [ ! -f .env ]; then
    ...
    # The following variables are commented out as they're not needed
    ...
    if [[ "$setup_domains" == "y" || "$setup_domains" == "Y" ]]; then
        ...
        # Uncomment and configure Caddy section in .env for n8n and Portainer only
        sed -i 's/^#N8N_HOSTNAME=.*/N8N_HOSTNAME=n8n.'$BASE_DOMAIN'/' .env
        # The following are commented out as they're not needed
...
# Comment out the GPU choice section as we're not using Ollama
...
# Instead of using GPU profiles, we'll start services without Ollama
echo "Starting services without Ollama..."
python3 start_services.py --profile none
